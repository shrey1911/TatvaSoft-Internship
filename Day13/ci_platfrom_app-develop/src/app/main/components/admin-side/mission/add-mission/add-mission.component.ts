...
// Add this inside getMissionThemeList()
getMissionThemeList() {
  const getMissionThemeListSubscription = this._service.getMissionThemeList().subscribe((data: any) => {
    if (data.result == 1) {
      ...
      // 👇 Set default theme here (e.g., first one in the list)
      if (this.missionThemeList.length > 0) {
        this.addMissionForm.patchValue({
          missionThemeId: this.missionThemeList[0].value
        });
      }
